...
object ConfigManager {
    ...
    val fields: List<CfgField> = listOf(
        ...
        CfgField("server.allowRemoteMediaControl", CfgKind.BOOL, emptyList(), "Allow remote media playback control from mobile clients",
            { it.app.allowRemoteMediaControl }, { s, v -> appCopy(s) { copy(allowRemoteMediaControl = coerceBool("server.allowRemoteMediaControl", v)) } }),
        CfgField("net.mediaControlPort", CfgKind.PORT, emptyList(), "Remote media control UDP port",
            { it.app.mediaControlPort.toIntOrNull() ?: 9095 }, { s, v -> appCopy(s) { copy(mediaControlPort = coercePort("net.mediaControlPort", v).toString()) } })
    )
