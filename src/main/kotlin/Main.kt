...
object NetworkHandler_v1 {
    ...
    private fun startServerInstanceLocked(
        ...
        startDonationTimer()
        val mediaControlPortInt = SettingsRepository.loadSettings().app.mediaControlPort.toIntOrNull() ?: 9095
        MediaControlListener.start(mediaControlPortInt)
...
fun startGuiApplication(cliArgs: CliArgs) = application {
    ...
    // Start media control listener immediately at app launch so phone can always control PC
    LaunchedEffect(Unit) {
        val port = SettingsRepository.loadSettings().app.mediaControlPort.toIntOrNull() ?: 9095
        MediaControlListener.start(port)
    }
